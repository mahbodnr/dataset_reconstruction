...
def data_extraction(args, dataset_loader, model):
    ...
    l_freeze_mask = torch.zeros_like(l, dtype=bool)
    x_freeze_mask = torch.zeros_like(x, dtype=bool)
    ...
    for epoch in range(args.extraction_epochs):
        ...
        loss, kkt_loss, loss_verify, loss_primal = calc_extraction_loss(
            args, l, model, values, x, y
        )
        # print(type(loss))
        # print(loss)
        # print('kkt_loss shape: ', kkt_loss.shape)
        # print('loss_verify shape: ', loss_verify.shape)
        ...
        l_copy = l.clone()
        l_copy[l_freeze_mask] *= 0.0
        l.data = l_copy.data

        x_copy = x.clone()
        x_copy[x_freeze_mask] *= 0.0
        x.data = x_copy.data

        if epoch % args.extraction_evaluate_rate == 0:
            if epoch > 0:
                prop = max(epoch / args.extraction_epochs - 0.1, 0)
                dists = torch.abs(y * values - args.margin)
                sorted_values, sorted_indices = torch.sort(dists, descending=True)
                l_freeze_mask[
                    sorted_indices[0 : int(prop * len(sorted_indices))]
                ] = True
                x_freeze_mask[
                    sorted_indices[0 : int(prop * len(sorted_indices))]
                ] = True
            ...
            extraction_score = evaluate_extraction(
                args, epoch, kkt_loss, loss_verify, loss_primal, x, x0, y0, ds_mean
            )
